...
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "data" / "processed" / "personal_finance_chatml_messages.jsonl"
)
...
def csv_to_jsonl(input_path: Path, output_path: Path) -> None:
    """
    Converts CSV into ChatML JSONL format with only the messages field.
    """
    ...
    with input_path.open(
        ...
        # We only need system, user, assistant for the messages, but we'll keep others for validation
        ...
        missing_columns = required_for_validation - header_columns
        if missing_columns:
        ...
        for row_number, raw_row in enumerate(reader, start=2):
            try:
                ...
                # Create record with ONLY the messages field
                record = {
                    "messages": [
